...
export default function ExperienceSection() {
  ...
  return (
    <section id="trai-nghiem" className="py-20 sm:py-32 relative overflow-hidden bg-[#08111c]">
      ...
      <div className="ambient-glow w-[450px] h-[450px] bg-blue-900/20 top-1/4 -left-48" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-start">
          ...
          <div className="lg:col-span-6 flex flex-col space-y-4 sm:space-y-5">
            {experience.zones.map((zone, idx) => (
              ...
              >
                <div className="luxury-card rounded-md p-6 sm:p-7 border border-white/10 relative group">
                  <div className="flex items-center justify-between mb-2.5">
                    <span className="text-[10px] sm:text-xs tracking-[0.2em] font-semibold text-[#cba864] uppercase font-sans">
                      {zone.number}
                    </span>
                    {zone.badge && (
                      <span className="text-[10px] text-slate-300 bg-white/5 px-2.5 py-0.5 rounded-full border border-white/10">
                        {zone.badge}
                      </span>
                    )}
                  </div>
                  <h3 className="font-serif text-lg sm:text-xl text-white font-medium mb-2 group-hover:text-[#e8d098] transition-colors duration-300">
                    {zone.title}
                  </h3>
